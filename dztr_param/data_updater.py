...
def update_extended_data(extended_data, grand_ids_to_update, folder='ResDataInflowStorageRelease'):
    for entry in grand_ids_to_update:
        if isinstance(entry, tuple) and len(entry) == 3:
            grand_id, res_name, dam_name = entry
        elif isinstance(entry, int):
            grand_id = entry
            res_name = None
            dam_name = None
        else:
            raise ValueError("Invalid entry format. Each entry should be either an int or a tuple of length 3.")

        resdata = check_and_read_files(grand_id, folder)
        ...
        if resdata is not None:
            ...
            if grand_id not in extended_data['GRAND_ID'].values:
                new_row = {'GRAND_ID': grand_id}
                if res_name is not None:
                    new_row['RES_NAME'] = res_name
                if dam_name is not None:
                    new_row['DAM_NAME'] = dam_name
